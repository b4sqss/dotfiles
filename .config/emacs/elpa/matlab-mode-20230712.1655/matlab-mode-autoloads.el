...
Matlab-Maint minor mode is enabled in all buffers where `(lambda nil Should we
turn on in this buffer? Only if in the project. (let ((dir (expand-file-name
default-directory)) (ml (file-name-directory (expand-file-name (locate-library
matlab))))) (when (string= ml (substring dir 0 (min (length dir) (length ml))))
...
Matlab-Shell-Gud minor mode is enabled in all buffers where `(lambda nil Should
we turn on in this buffer? Only if in a MATLAB mode. (when (eq major-mode
'matlab-mode) (matlab-shell-gud-minor-mode 1)))' would do it.

See `matlab-shell-gud-minor-mode' for more information on Matlab-Shell-Gud minor
mode.
...
With prefix ARG, enable Global Matlab-Shell-Inactive-Gud minor mode if ARG is
...
Matlab-Shell-Inactive-Gud minor mode is enabled in all buffers where `(lambda
nil Should we turn on in this buffer? Only if in a MATLAB mode. (when (eq
major-mode 'matlab-mode) (matlab-shell-inactive-gud-minor-mode 1)))' would do
it.
